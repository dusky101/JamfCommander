    ...
    private var modules: some View {
        InfoSection(title: "What each section does", icon: "square.grid.2x2") {
            ...
            moduleRow("Add PKG", "arrow.up.doc.fill",
                      "For software Installomator has no label for. Drop a **.pkg**, **.mpkg**, **.dmg** or **.zip** onto the page, fill in the package details, and the app uploads it to Jamf and creates the Self Service install policy — the same category, scope, Self Service and icon choices the Packages module offers. The upload shows progress and can be cancelled. The **Uploaded** and **Deployed** tabs list the packages already in Jamf — Deployed shows only those a policy installs, worked out by reading every policy, so it takes a moment the first time. Needs the **Create**, **Read** and **Update Packages** privileges.")

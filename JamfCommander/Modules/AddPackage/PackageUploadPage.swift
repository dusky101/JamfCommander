//
//  PackageUploadPage.swift
//  JamfCommander
//
//  The "New" tab of the Add PKG module: uploads a package an administrator supplies — a .pkg or .dmg
//  that Installomator has no label for — and creates the Self Service install policy for it, with the
//  same category, scope, Self Service and icon choices the Installomator flow offers.
//
//  Reference data (categories, computers, groups, the existing package and policy names) belongs to
//  the module container, `AddPackageView`, because the library tabs need it too. This page owns only
//  the file being uploaded and the form around it.
//
...
struct PackageUploadPage: View {
    @ObservedObject var api: JamfAPIService

    /// Loaded once by the module container and shared with the library tabs.
    let categories: [Category]
    let computers: [ComputerInventoryRecord]
    let computerGroups: [ComputerGroup]
    let existingPackageNameKeys: Set<String>
    let existingPolicyNameKeys: Set<String>
    /// True when the package-name check could not run, so the page says so rather than implying a
    /// name is free.
    let nameCheckFailed: Bool

    /// A category was created here — the container takes the refreshed list so every tab agrees.
    var onCategoriesReloaded: ([Category]) -> Void
    /// A package actually reached Jamf, so the library and the name indexes are now stale.
    ...
    private func performUpload() {
        ...
        runTask = Task {
            ...
            do {
                ...
                await MainActor.run { finish(steps, uploaded: false) }
                return
            }
            ...
            guard createsPolicy else {
                await MainActor.run { finish(steps, uploaded: true) }
                return
            }
    ...
    @MainActor
    private func finish(_ steps: [OperationResult], uploaded: Bool) {
        stage = .idle
        runTask = nil
        results = steps
        showResults = true
    ...
    private func createCategory() {
        ...
        Task {
            ...
            await MainActor.run {
                if let fresh {
                    onCategoriesReloaded(fresh.sorted {
